...
function sisgp_sl_api_login_auth( $user, $username, $password ){
    ...
    if($username != ""){
        ...
        $log_user_mail = get_user_by('email', $username);
        $log_user_login = get_user_by('login', $username);

        // Verificar se entrou com credenciais válidas
        $userLog = $log_user_mail instanceof WP_User ? $log_user_mail : $log_user_login;
        if($userLog instanceof WP_User){ // Caso não seja um erro
            if(!$userLog->has_cap('externo')){ // É um login "nativo"
                $user = wp_authenticate_username_password($userLog->user_login, $password);
                // Se conseguiu autenticar, assume login
                if($user instanceof WP_User){
                    { toLog("sisgpsl_api_login_PASS=> OK."); }
                    return $user;
                }
            } else {
            ...
            { toLog("sisgpsl_api_login_PASS_fail=> Login incorreto."); }
        }
    ...
    if($username == '' || $password == '') return $user; // Caso erro, informe o erro.

    

    // É um login, mas autenticador do WP detectou erro...
    if(is_wp_error($user)){
        $senha = md5($password);
        { toLog("wp_auth_err=> ".$user->get_error_message()); }
    } else // Ou é um login da API:
        {
            $senha = $password; // Não vai precisar de encode
            toLog("wp_auth_USER=> ".print_r($user,true));
        }
    ...
    $baseurl=get_option('sisgpsl_api_login_text_urlbase');
    $ambiente=get_option('sisgpsl_api_login_text_ambiente');
    $res = api_fluxus_login($baseurl,$ambiente,$username, $senha);
    {toLog("sisgpsl_api_CURL=> ".json_encode($res));}

    $FALHOU = true;
    if($res)
        if(array_key_exists("userName", $res)) { // Login trouxe um username (provável sucesso)

            if($res["userName"] == $username) { // Login e usuário idênticos. Sucesso!
                { toLog("sisgpsl_api_login_SUCCESS"); }
                // Vamos descobrir se o usuário existe:
                $user = get_user_by( 'login', $username );


                if ($user instanceof WP_User) { // Se foi encontrado ID válido

                    { toLog("sisgpsl_api_login_SUCCESS_LOGGED"); }
                    $FALHOU = false;
                    ...
                    $FALHOU = false;
                    $params = sprintf("?u=%s&h=%s&k=%s&a=%d", $username, $senha, $res["token"], get_current_user_id());
                    {toLog("sisgpsl_WP_primeiroLogin->reqFormRegister = ".get_option('sisgpsl_api_login_url_cadastrofinal').$params);}
                    wp_redirect( get_option('sisgpsl_api_login_url_cadastrofinal').$params );
                    exit(1);
        ...
        // Caso falhe nos outros casos
        if($FALHOU){
            if(is_wp_error( $user )){
                { toLog("wp_auth_err_FINAL_UNEXCEPTED=> ".$user->get_error_message()); }
                return $user;
            } else {
                { toLog("wp_auth_err_FINAL_UNEXCEPTED=> ???"); }
